...
import '../driver_v2/driver_v2_verify_dialog.dart';

class DriverBfV2Panel extends StatefulWidget {
  final String searchQuery;
  const DriverBfV2Panel({super.key, required this.searchQuery});

  @override
  State<DriverBfV2Panel> createState() => _DriverBfV2PanelState();
}

class _DriverBfV2PanelState extends State<DriverBfV2Panel> {
  List<Map<String, dynamic>> _deliversList = [];
  StreamSubscription? _deliversSubscription;
  ...
  void _startRealtime() {
    _deliversSubscription = Supabase.instance.client
        .from('deliveries')
        .stream(primaryKey: ['id_delivery'])
        .listen((data) {
      if (mounted) {
        setState(() {
          _deliversList = List<Map<String, dynamic>>.from(data);
        });
      }
    });
  }
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, dark, child) {
        ...
        if (widget.searchQuery.trim().isNotEmpty) {
          final sq = widget.searchQuery.trim().toLowerCase();
          items = items.where((u) {
            final company = (u['company']?.toString() ?? '').toLowerCase();
            final driver = (u['driver_name']?.toString() ?? '').toLowerCase();
            final door = (u['door']?.toString() ?? '').toLowerCase();
            final type = (u['type']?.toString() ?? '').toLowerCase();
            return company.contains(sq) || driver.contains(sq) || door.contains(sq) || type.contains(sq);
          }).toList();
        }
