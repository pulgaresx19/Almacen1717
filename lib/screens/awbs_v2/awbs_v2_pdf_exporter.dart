...
class AwbsV2PdfExporter {
  ...
  static void _addDetailedAwbPage(pw.Document pdf, Map<String, dynamic> u) {
    ...
    // Compute details for V2
    String awbNumber = u['awb_number']?.toString() ?? '-';
    ...
    int receivedPieces = int.tryParse(u['total_pieces']?.toString() ?? '0') ?? 0;

    int deliveredPieces = 0; // Legacy delivery not ported to V2 yet
    List deliverHistory = []; // Legacy delivery history not ported to V2 yet
    
    int remainingPieces = expectedPieces - deliveredPieces;
    if (remainingPieces < 0) remainingPieces = 0;

    String status = 'WAITING'; // Legacy status not ported to V2 yet
